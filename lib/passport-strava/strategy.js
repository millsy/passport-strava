  ...
  this._oauth2.get('https://www.strava.com/api/v3/athlete', accessToken, function (err, body, res) {
    ...
    try {
      ...
      if (json.athlete) {
        ...
        profile.id = json.athlete.id;
        profile.displayName = json.athlete.firstname + ' ' + json.athlete.lastname;
		profile.name = { familyName : json.athlete.lastname, givenName : json.athlete.firstname};
		profile.photos = [{ value : json.athlete.profile }, { value : json.athlete.profile_medium }]
        profile.emails = [{ value: json.athlete.email }];
      }
